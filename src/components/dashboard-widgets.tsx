...
import { Activity, ArrowDownRight, ArrowUpRight, Bookmark, Bot, Filter, Gauge, Layers3, Newspaper, Radio, Search, SlidersHorizontal, Sparkles, Volume2 } from "lucide-react";
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <h3 className="font-semibold text-white">สรุปตลาดวันนี้</h3>
          </div>
        </div>
        <StatusPill tone="up">Bullish bias</StatusPill>
      </div>
      <p className="mt-4 text-sm leading-6 text-slate-300">
        ตลาดยังให้น้ำหนักหุ้น AI, semiconductor และ space infrastructure หลัง volume กลับเข้ากลุ่ม growth.
        ความเสี่ยงหลักคือ CPI/FED minutes และ valuation ที่ตึงใน megacap AI.
      </p>
  ...
  return (
    <Panel className="p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <StatusPill tone="info">{quotes.length} symbols</StatusPill>
      </div>
        ...
        {rows.map((quote) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <div>
                  <strong className="font-mono text-white">{quote.ticker}</strong>
  ...
  return (
    <Panel className="p-4">
      <div className="flex items-center justify-between">
        ...
        <StatusPill tone="info">S&P · Nasdaq · SET</StatusPill>
      </div>
        ...
        {heatmap.map((stock) => {
          ...
          return (
            <motion.button
              ...
              className={`rounded-md p-3 text-left ${up ? "bg-emerald-400/15 hover:bg-emerald-400/25" : "bg-rose-400/15 hover:bg-rose-400/25"}`}
              style={{ minHeight: `${54 + stock.size}px` }}
            >
              <div className="font-mono text-sm text-white">{stock.ticker}</div>
              <div className={up ? "text-emerald-300" : "text-rose-300"}>{up ? "+" : ""}{stock.change}%</div>
              <div className="mt-2 text-xs text-slate-500">{stock.sector}</div>
            </motion.button>
          );
        })}
      </div>
...
export function CopilotWidget() {
  const prompts = ["วิเคราะห์ NVDA ตอนนี้", "พอร์ตเสี่ยงไหม", "หุ้นไหนกำลัง breakout", "สรุปข่าว RKLB ภาษาไทย"];
