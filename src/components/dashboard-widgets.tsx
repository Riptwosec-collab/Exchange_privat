...
import { ArrowDownRight, ArrowUpRight, Bookmark, Bot, Filter, Gauge, Newspaper, SlidersHorizontal, Sparkles } from "lucide-react";
import { motion } from "framer-motion";
import { Area, AreaChart, Cell, Pie, PieChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";
import { economicEvents, gainers, heatmap, indices, losers, news, portfolio } from "@/lib/mock-data";
...
export function HeatmapPanel() {
  return <Panel className="p-4"><div className="flex items-center justify-between"><h3 className="font-semibold text-white">Market Heatmap</h3><StatusPill tone="info">S&P · Nasdaq · SET</StatusPill></div><div className="mt-4 grid grid-cols-4 gap-2">{heatmap.map((stock) => { const up = stock.change >= 0; return <motion.button key={stock.ticker} whileHover={{ scale: 1.03 }} className={`rounded-md p-3 text-left ${up ? "bg-emerald-400/15 hover:bg-emerald-400/25" : "bg-rose-400/15 hover:bg-rose-400/25"}`} style={{ minHeight: `${54 + stock.size}px` }}><div className="font-mono text-sm text-white">{stock.ticker}</div><div className={up ? "text-emerald-300" : "text-rose-300"}>{up ? "+" : ""}{stock.change}%</div><div className="mt-2 text-xs text-slate-500">{stock.sector}</div></motion.button>; })}</div></Panel>;
}

export function ScreenerPanel() {
  const quotes = useMarketStore((state) => state.quotes);
  return <Panel className="p-4"><div className="flex items-center justify-between"><h3 className="font-semibold text-white">AI Stock Screener</h3><SlidersHorizontal size={18} className="text-slate-400" /></div><div className="mt-3 flex flex-wrap gap-2">{["Market Cap", "P/E", "Revenue Growth", "RSI", "Breakout", "AI Stocks", "Dividend", "Momentum"].map((filter) => <button key={filter} className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-slate-300 hover:border-cyan-300/40">{filter}</button>)}</div><div className="mt-4 overflow-x-auto scrollbar-thin"><table className="w-full min-w-[540px] text-sm"><thead className="text-left text-xs uppercase tracking-[0.16em] text-slate-500"><tr><th className="py-2">Ticker</th><th>Sector</th><th>Price</th><th>RSI</th><th>Volume</th><th>Signal</th></tr></thead><tbody>{quotes.map((quote) => <tr key={quote.ticker} className="border-t border-white/10"><td className="py-3 font-mono text-white">{quote.ticker}</td><td className="text-slate-400">{quote.sector}</td><td className="font-mono text-slate-200">${quote.price.toFixed(2)}</td><td className="font-mono text-slate-200">{quote.rsi}</td><td className="font-mono text-slate-400">{quote.volume}</td><td><StatusPill tone={quote.rsi > 65 ? "up" : "neutral"}>{quote.rsi > 65 ? "Breakout" : "Watch"}</StatusPill></td></tr>)}</tbody></table></div></Panel>;
}

export function MultiChartPanel() {
  return <Panel className="p-4"><div className="flex items-center justify-between"><h3 className="font-semibold text-white">Multi Chart Monitor</h3><StatusPill tone="info">Sync timeframe</StatusPill></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{["NVDA", "TSLA", "RKLB", "AMD"].map((ticker, index) => <div key={ticker} className="rounded-md border border-white/10 bg-black/20 p-3"><div className="mb-2 flex items-center justify-between"><span className="font-mono text-white">{ticker}</span><span className={index === 1 ? "text-rose-300" : "text-emerald-300"}>{index === 1 ? "-1.67%" : "+2.1%"}</span></div><div className="flex h-24 items-end gap-1">{Array.from({ length: 34 }, (_, i) => <span key={i} className="flex-1 rounded-t bg-cyan-300/40" style={{ height: `${(24 + Math.abs(Math.sin(i + index)) * 70).toFixed(2)}%` }} />)}</div></div>)}</div></Panel>;
}

export function CalendarAndFlows() {
  return <Panel className="p-4"><div className="flex items-center gap-2"><Gauge size={18} className="text-purple-300" /><h3 className="font-semibold text-white">Macro · Insider · Social</h3></div><div className="mt-4 space-y-3">{economicEvents.map((event) => <div key={event.event} className="grid grid-cols-[1fr_auto] gap-3 rounded-md border border-white/10 bg-white/[0.035] p-3"><div><p className="text-sm font-medium text-white">{event.event}</p><p className="text-xs text-slate-500">{event.date} · Forecast {event.forecast}</p></div><StatusPill tone={event.impact === "High" ? "down" : "neutral"}>{event.impact}</StatusPill></div>)}</div></Panel>;
}
...
export function CopilotWidget() {
  return <Panel className="fixed bottom-4 right-4 z-50 hidden w-[360px] p-4 xl:block"><div className="flex items-center gap-2"><div className="flex h-9 w-9 items-center justify-center rounded-md bg-purple-400/15 text-purple-200"><Sparkles size={18} /></div><div><h3 className="text-sm font-semibold text-white">AI Stock Copilot</h3><p className="text-xs text-slate-500">Thai explanation · trader memory</p></div></div><div className="mt-4 rounded-md border border-white/10 bg-black/30 p-3 text-sm leading-6 text-slate-300">NVDA ยังเป็น leader กลุ่ม AI แต่ควรจับตา RSI และ earnings guidance. แนวรับใกล้ 136-138, แนวต้าน 151-154.</div><div className="mt-3 flex flex-wrap gap-2">{["วิเคราะห์ NVDA ตอนนี้", "พอร์ตเสี่ยงไหม", "หุ้นไหนกำลัง breakout", "สรุปข่าว RKLB ภาษาไทย"].map((prompt) => <button key={prompt} className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-slate-300 hover:border-purple-300/40">{prompt}</button>)}</div></Panel>;
}
