    ...
    <div className="relative mt-3 h-36 overflow-hidden rounded-md border border-white/10 bg-black/30">
      {mode === "Line" || mode === "Area" ? (
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full">
          ...
          <polyline points={points} fill="none" stroke={quote.changePercent >= 0 ? "#22c55e" : "#fb7185"} strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" vectorEffect="non-scaling-stroke" />
        </svg>
      ) : null}
      {mode === "Range" ? (
      ...
      {mode !== "Line" && mode !== "Area" && mode !== "Range" ? (
        <div className="absolute inset-x-3 bottom-3 top-3 flex items-end gap-1">
          {data.map((item, index) => {
            ...
            if (mode === "Volume") return <span key={index} className={`flex-1 rounded-t ${up ? "bg-[#22c55e]/75" : "bg-[#fb7185]/70"}`} style={{ height: `${100 - top}%` }} />;
